---
layout: '@layouts/Docs.astro'
title: Installation
description: corvu is a collection of unstyled, accessible and customizable UI primitives for SolidJS
image: https://corvu.dev/banner.jpg
---
import Code from '@components/docs/code/Code.astro'
import H2 from '@components/docs/headings/H2.astro'
import H3 from '@components/docs/headings/H3.astro'
export const components = { h2: H2, h3: H3 }

# Installation
corvu provides every UI primitive as a separate package. This allows you to only install the primitives you need, to use the semver of every package as a reference for breaking/feature changes and progressively update packages in case of new versions.

For example the drawer component is available as `@corvu/drawer`:

<Code code={`
  npm install @corvu/drawer
`} lang="bash" copyButton />

If you want to install all primitives at once you can install the main package. Unused primitives will get tree-shaken by your bundler, so you don't have to worry about the bundle size.

<Code code={`
  npm install corvu
`} lang="bash" copyButton />

You're good to go! Head over to [Usage](/docs/state/) to learn how to use corvu.

## Tailwind CSS plugin

If you want make use of the tailwind modifiers like `corvu-open:`, install the tailwind plugin:

<Code code={`
  npm install @corvu/tailwind
`} lang="bash" copyButton />

Then add the plugin to your `tailwind.config.js` file:

<Code code={`
  module.exports = {
    // ...
    plugins: [
      // Use it with the default prefix 'corvu'
      require('@corvu/tailwind'),
      // or with a custom prefix
      require('@corvu/tailwind')({ prefix: 'ui' }),
      // ...
    ],
  }
`} lang="js" />

Check out the [Styling](/docs/styling/) guide learn how to use the tailwind plugin.

### Tailwind v4.0
Tailwind CSS v4.0 improved working with data attributes a lot and this plugin isn't considered useful anymore. You can target corvu's data attributes directly (eg. `data-open:` or `group-data-open:`).

## UnoCSS preset
We also provide a preset for [UnoCSS](https://unocss.dev/) that adds the same modifiers.

<Code code={`
  npm install @corvu/unocss
`} lang="bash" copyButton />

Add the preset to your `uno.config.tsx` file:

<Code code={`
  import { defineConfig } from 'unocss'
  import presetCorvu from '@corvu-next/unocss'

  export default defineConfig({
    // ...
    presets: [
      // Use it with the default prefix 'corvu'
      presetCorvu(),
      // or with a custom prefix
      presetCorvu({ prefix: 'ui' }),
      // ...
    ],
  })
`} lang="ts" />

Check out the [Styling](/docs/styling/) guide learn how to use the UnoCSS preset.

